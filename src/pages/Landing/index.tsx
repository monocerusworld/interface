...
const Glow = styled.div<{ bgColor: string }>`
  position: absolute;
  top: 68px;
  bottom: 0;
  background: ${({ bgColor }) =>
...
const ButtonCTA = styled(LandingButton)`
  ...
  &:hover {
    box-shadow: 0px 0px 16px 0px #918863;
  }
`;
...
export default function Landing() {
  ...
  const ch = useWeb3React();
  const { infoLink, backgroundColor, color } = getChainInfoOrDefault(
    ch.chainId
  );
        ...
        <PageContainer isDarkMode={isDarkMode} data-testid="landing-page">
          ...
          <AboutContentContainer isDarkMode={isDarkMode}>
            <CardGrid cols={2} ref={cardsRef}>
              {MAIN_CARDS.map(
                ({ darkBackgroundImgSrc, lightBackgroundImgSrc, ...card }) => (
                  <Card
                    {...card}
                    backgroundImgSrc={
                      isDarkMode ? darkBackgroundImgSrc : lightBackgroundImgSrc
                    }
                    key={card.title}
                  />
                )
